...
function renderJiraSection(scope) {
    ...
    container.innerHTML = `
        <div class="jira-section">
            ...
            <ul class="${itemsClass}" id="jiraItems">${itemsHtml}</ul>
        </div>`;
}
...
function renderJiraTicketItem(ticket) {
    ...
    return `
        <li class="${itemClass}" onclick="openJiraTicket('${escapeHtml(ticket.url)}')">
            <span class="jira-item-key">${escapedKey}</span>
            ...
            <span class="${bellClass}" onclick="event.stopPropagation();openJiraReminderPicker('${escapedKey}','${escapedReminderAt}')">${bellIcon}</span>
        </li>`;
}
